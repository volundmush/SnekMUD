...
from snekmud.utils import callables_from_module, variable_from_module, pad, crop, justify, safe_convert_to_types
from snekmud import GETTERS, OPERATIONS, COMPONENTS, WORLD
    ...
    def validate_callables(self, callables):
        ...
        for funcname, clble in callables.items():
            try:
                ...
                logging.error(f"Could not run getfullargspec on {funcname}: {clble}")
            else:
...
def funcparser_callable_you(
    *args, caller=None, receiver=None, mapping=None, capitalize=False, **kwargs
    ...
    return GETTERS["GetDisplayName"](receiver, caller).execute()
...
def funcparser_callable_pronoun(*args, caller=None, receiver=None, capitalize=False, **kwargs):
    ...
    if (gen := GETTERS["Gender"](receiver, caller).execute()):
        default_gender = gen
